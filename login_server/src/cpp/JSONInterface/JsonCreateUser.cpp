...
Document JsonCreateUser::handle(const Document& params)
{
	...
	if (password.size()) {
		...
		if (!sm->checkPwdValidation(password, &errors, LanguageManager::getInstance()->getFreeCatalog(languageKey))) {
	...
	if (languageKey != LANG_NULL) {
		userModel->setLanguageKey(language);
	}
	getIntParameter(params, "publisher_id", publisherId);
	if (publisherId > 0) {
		userModel->setPublisherId(publisherId);
	}

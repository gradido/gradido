...
void grdw_community_root_assemble(
  ...
  memcpy(community_root->pubkey, pubkey, ED25519_PUBLIC_KEY_SIZE);
  memcpy(community_root->gmw_pubkey, gmw_pubkey, ED25519_PUBLIC_KEY_SIZE);
  memcpy(community_root->auf_pubkey, auf_pubkey, ED25519_PUBLIC_KEY_SIZE);
}
...
void grdw_gradido_creation_assemble(
  ...
  memcpy(gradido_creation->recipient.pubkey, recipient_pubkey, ED25519_PUBLIC_KEY_SIZE);
...
void grdw_gradido_transfer_assemble(
  ...
  memcpy(gradido_transfer->sender.pubkey, sender_pubkey, ED25519_PUBLIC_KEY_SIZE);
  gradido_transfer->sender.amount = amount;
  memcpy(gradido_transfer->sender.community_uuid, community_uuid, UUID_BINARY_SIZE);
  memcpy(gradido_transfer->recipient, recipient_pubkey, ED25519_PUBLIC_KEY_SIZE);
}
...
void grdw_register_address_assemble(
  ...
  memcpy(register_address->user_pubkey, user_pubkey, ED25519_PUBLIC_KEY_SIZE);
  register_address->address_type = address_type;
  register_address->derivation_index = derivation_index;
  memcpy(register_address->name_hash, name_hash, BLAKE2B_HASH_SIZE);
  memcpy(register_address->account_pubkey, account_pubkey, ED25519_PUBLIC_KEY_SIZE);
}

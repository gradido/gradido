...
async function declareProjectAccount(user: DbUser): Promise<VoidResult<ProjectAccountRefused>> {
  ...
  const written = await dbSetCreationAllowed(user.id, false)
  if (!written.success) {
  ...
  await EVENT_PROJECT_ACCOUNT_DECLARE(user)
  return { success: true }
}
 ...
 * "one a day" read below and send two mails. The event is written BEFORE the mail goes out:
 * a mail that fails to send is retried by the member tomorrow, a mail that went out twice
...
async function requestCreationRight(
  ...
  try {
    ...
    const request = await EVENT_CREATION_RIGHT_REQUEST(user)
    await sendCreationRightRequestSupportEmail({

...
export class UserResolver {
  ...
  async login(
    ...
    if (!dbUser.emailChecked) {
      logger.error('The Users email is not validate yet.') // Wolle: put description here
      throw new Error(ERRORS[ERRORS.ERR_EMAIL_NOT_VALIDATED])
    }
    if (dbUser.password === BigInt(0)) {
      logger.error('The User has not set a password yet.') // Wolle: put description here
  ...
  async createUser(
    ...
    if (userFound) {
      logger.error('User already exists with this email=' + email) // Wolle: put description here

...
const asPreFieldStock = async (memo: string): Promise<void> => {
  // Submitting always stamps creation_groups_set_at. Clearing it reproduces stock from before
  ...
  await DbContribution.update({ memo }, { creationGroupsSetAt: null })
}
...
beforeAll(async () => {
  ...
  await DbCreationGroup.save([
    DbCreationGroup.create({ tag: 'feuerwehr', name: 'Feuerwehr' }),
    DbCreationGroup.create({ tag: 'hexen', name: 'Hexen' }),
    DbCreationGroup.create({ tag: 'chor', name: 'Chor' }),
  ])
})
...
describe('the member has said something themselves', () => {
  ...
  it('suggests the group of the most recent contribution', async () => {
    ...
    expect((await suggestCreationGroupForUser(member.id))?.tag).toBe('chor')
  })

  it('follows a change of group', async () => {
    await submit('suggestion: for the fire brigade now', ['feuerwehr'])
    expect((await suggestCreationGroupForUser(member.id))?.tag).toBe('feuerwehr')
  })

  it('honours a deliberate "no group" and does not reach past it', async () => {
    // The decisive case. Without the creation_groups_set_at stamp this would keep suggesting
    ...
    expect(await suggestCreationGroupForUser(member.id)).toBeNull()
  })
  ...
  it('walks past pre-field stock that says nothing, without falling through to the main tag', async () => {
    ...
    await saveUserCreationGroups(member.id, ['chor'])
...
describe('the member said something once, long ago', () => {
  ...
  it('suggests that group again', async () => {
    ...
    expect((await suggestCreationGroupForUser(member.id))?.tag).toBe('hexen')
  })
  ...
  it('lets that old choice outrank a main tag a moderator entered', async () => {
    ...
    await saveUserCreationGroups(member.id, ['feuerwehr'])
    expect((await suggestCreationGroupForUser(member.id))?.tag).toBe('hexen')
  })
})
...
describe('the member has never said anything', () => {
  ...
  it('falls back to the main tag a moderator entered', async () => {
    ...
    await saveUserCreationGroups(member.id, ['feuerwehr', 'chor'])
    expect((await suggestCreationGroupForUser(member.id))?.tag).toBe('feuerwehr')
  })
})

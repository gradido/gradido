 ...
 * Its timestamp is what the support mail later names as "declared at" — which is why it is
 * written inside the caller's transaction, together with the column: neither without the
 * other.
 */
export const EVENT_PROJECT_ACCOUNT_DECLARE = async (
  user: DbUser,
  manager?: EntityManager,
): Promise<DbEvent> => {
  const event = Event(EventType.PROJECT_ACCOUNT_DECLARE, user, user)

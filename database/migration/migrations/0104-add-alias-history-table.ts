...
export async function upgrade(queryFn: (query: string, values?: any[]) => Promise<Array<any>>) {
  ...
  await queryFn(`ALTER TABLE users ADD alias_startupdate_at datetime(3) DEFAULT NULL after alias;`)
  await queryFn(
...
export async function downgrade(queryFn: (query: string, values?: any[]) => Promise<Array<any>>) {
  ...
  for (const user of users) {
    ...
    if (
      ...
      // console.log(
      //   `Alias matches generated alias pattern alias=${user.alias}, generated=${generatedAlias}`,
      // )
      ...
      // console.log(
      //   `Alias does not match generated alias pattern alias=${user.alias}, generated=${generatedAlias}`,
      // )
    }
  }
}

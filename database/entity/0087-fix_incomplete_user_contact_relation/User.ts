...
export class User extends BaseEntity {
  ...
  // Cascade insert here means if there is a new UserContact instance set
  // on this relation, it will be inserted automatically to the db when you save this User entity
  ...
  public getPrimaryUserContact(): UserContact {
    const primaryContact = this.userContacts?.find((userContact) => userContact.isPrimary)
    if (!primaryContact) {
      throw new Error(
        'cannot find primary user contact for user' +
          JSON.stringify({
            userId: this.id,

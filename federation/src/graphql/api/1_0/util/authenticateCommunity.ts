...
const createLogger = (method: string) =>
  getLogger(`${LOG4JS_BASE_CATEGORY_NAME}.graphql.api.1_0.util.authenticateCommunity.${method}`)
...
export async function startOpenConnectionCallback(
  ...
  try {
    const pendingState = await findPendingCommunityHandshake(
      publicKey,
      api,
      CommunityHandshakeStateType.START_OPEN_CONNECTION_CALLBACK,
    )
    if (pendingState) {
      ...
      if (!(await stateLogic.isTimeoutUpdate())) {
        ...
        methodLogger.debug(
          ...
          new CommunityHandshakeStateLoggingView(pendingState),
        )
        return
      }
    }
...
export async function startAuthentication(
  ...
  try {
    ...
    state = await findPendingCommunityHandshake(
      fedComBPublicKey,
      fedComB.apiVersion,
      CommunityHandshakeStateType.START_COMMUNITY_AUTHENTICATION,
    )
    if (!state) {
    ...
    if (client instanceof V1_0_AuthenticationClient) {
      const authenticationArgs = new AuthenticationJwtPayloadType(
        handshakeID,
        oneTimeCode,
        homeComA!.communityUuid!,
      )
      ...
      if (responseJwt !== null) {
        const payload = (await verifyAndDecrypt(
          handshakeID,
          responseJwt,
          homeComA!.privateJwtKey!,
          comB.publicJwtKey!,
        )) as AuthenticationResponseJwtPayloadType
        /*methodLogger.debug(
        ...
        if (
          payload.tokentype !== AuthenticationResponseJwtPayloadType.AUTHENTICATION_RESPONSE_TYPE
        ) {
          throw new Error(

...
const glob_sync_1 = __importDefault(require("./glob-sync"));
...
function listFilesToProcess(globPatterns, providedOptions) {
    ...
    const resolvedPathsByGlobPattern = resolvedGlobPatterns.map(pattern => {
        ...
        return new glob_sync_1.default(pattern, globOptions, shouldIgnore).found.map(globMatch => {
    ...
    const ret = lodash_1.default.uniqBy(allPathDescriptors, pathDescriptor => pathDescriptor.filename);
    debug(ret);
    return ret;
}
